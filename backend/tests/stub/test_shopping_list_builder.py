...
def test_uses_generic_quantity_and_category():
    items = ShoppingListBuilder().build({"tomato": ["r1"]}, profile={"language": "en"})
    item = items[0]
...
def test_identity_translation_for_es():
    items = ShoppingListBuilder().build({"tomato": ["r1"]}, profile={"language": "es"})
    assert items[0]["ingredient_es"] == "tomato"


def test_preserves_attribution_recipe_ids():
    items = ShoppingListBuilder().build({"tomato": ["r1", "r2", "r4"]}, profile={"language": "en"})
...
def test_accepts_reasoner_kwarg_and_ignores_it():
    items = ShoppingListBuilder(reasoner=object()).build({"x": ["r1"]}, profile={"language": "en"})
